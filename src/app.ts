 ...
 *
 * `app` is a plain Hono instance — no Cloudflare/Vercel/Node-specific code
 * touches it. Every deployment target (src/index.ts for Workers, api/index.ts
 * for Vercel, src/server.ts for Docker/any-Node-host) is just a few lines
 * that hand this same instance its platform's `(request, env, ctx)` triple.
 *
 * `createFetchHandler` wraps `app.fetch` with the one bit of routing glue
 * every target needs: rewriting a bare `/` request to `/v4/` before it hits
 * the `.basePath("/v4")` router (otherwise `/` 404s, since nothing is
 * mounted outside the /v4 prefix).
 */
export interface FetchHandlerOptions {
  /**
   * Log every request/response to the console. Off by default — deliberately
   * NOT enabled for the Cloudflare Workers (src/index.ts) or Vercel
   * (api/index.ts) adapters, since both platforms already provide their own
   * request logging/observability (Workers Logs, Vercel's Function Logs),
   * and console output there is either discarded or billed differently than
   * a plain container/process. Node, Deno, and Bun adapters turn this on
   * since they otherwise have no logging at all.
   */
  logRequests?: boolean;
}
  ...
  return {
    async fetch(request: Request, env: any, ctx: any) {
      ...
      try {
        ...
        console.log(
          `[${new Date().toISOString()}] ${request.method} ${url.pathname}${url.search} ${status} ${ms}ms`,
        );
      }
    },
  };
